...
outfile = open(out, "w")
outfile.write(
    "Name of path\t Opt. id. [%]\t st. dev. [%]\t No. of paths\t st. dev.\t length [aa]\t st. dev.\t length [dist]\t st. dev.\t A_FL\t st. dev.\t B_F\t st. dev.\t A_E\t st. dev.\t B_ED\t st. dev.\n")
...
for i in range(len(lines) // reps):
    ...
    outfile.write(
        f"{name}\t{sim_avg}\t{sim_sd}\t{num_avg}\t{num_sd}\t{aa_avg}\t{aa_sd}\t{d_avg}\t{d_sd}\t{A_FL_avg}\t{A_FL_sd}\t{B_F_avg}\t{B_F_sd}\t{A_E_avg}\t{A_E_sd}\t{B_ED_avg}\t{B_ED_sd}\n")
